...
const PACKAGES = [
    {
        ...
        description: "Modular, type-safe React framework: state, forms, queries, pages, localization, auth, notifications",
    ...
    },
    {
        ...
        description: "Dynamic, type-safe form builder on TanStack React Form with Jotai state",
    ...
    },
    {
        ...
        description: "Unified data fetching layer on TanStack Query + Jotai: queries, mutations, WebSockets",
    ...
    },
    {
        ...
        description: "Page orchestrator: forms + queries + SEO + lazy loading + cross-platform",
...
function findPackage(id) {
    return PACKAGES.find((p) => p.id === id || p.name === id || p.id === id.replace(/^@gaddario98\//, ""));
}
// ─── MCP Server ─────────────────────────────────────────────────────────────────
...
for (const pkg of PACKAGES) {
    server.registerResource(`${pkg.id}-readme`, `docs://${pkg.id}/readme`, {
...
server.registerTool("list_packages", {
    description: "List all available @gaddario98 React packages with their descriptions and GitHub links",
}, async () => {
    // Determine version by fetching the root package.json
    let version = "unknown";
    try {
        const pkgJsonText = await fetchRawFile("gaddario98/react-core", "master", "package.json");
...
// 2. get_package_docs
server.registerTool("get_package_docs", {
    description: "Get the full README.md documentation for a specific @gaddario98 React package",
    inputSchema: {
        packageId: z
            .string()
            .describe("Package identifier: 'react-core', 'react-form', 'react-queries', or 'react-pages'"),
    },
}, async ({ packageId }) => {
...
// 3. get_package_types
server.registerTool("get_package_types", {
    description: "Get the TypeScript type definitions (types.ts) for a specific @gaddario98 React package",
    inputSchema: {
        packageId: z
            .string()
            .describe("Package identifier: 'react-form', 'react-queries', or 'react-pages'"),
    },
}, async ({ packageId }) => {
    const pkg = findPackage(packageId);
    if (!pkg) {
...
server.registerTool("get_package_source", {
    description: "Read the contents of a specific source file from a @gaddario98 React package. Use list_packages first to see available packages, then explore files.",
    inputSchema: {
        packageId: z
            .string()
            .describe("Package identifier: 'react-core', 'react-form', 'react-queries', or 'react-pages'"),
        filePath: z
            .string()
            .optional()
            .describe("Relative path to the file within the package directory (e.g., 'hooks/useFormManager.tsx'). Omit to list all source files."),
    },
    ...
    if (!filePath) {
        try {
            const treeData = await fetchGitHubAPI(`/repos/${pkg.repo}/git/trees/${pkg.branch}?recursive=1`);
            ...
            const files = treeData.tree
                ...
                .map((path) => pkg.basePath ? path.replace(pkg.basePath + "/", "") : path);
            return {
                content: [
                    {
                        type: "text",
...
// 5. search_source
server.registerTool("search_source", {
    description: "Search for a pattern across all source files of a @gaddario98 React package using GitHub Code Search. Requires GITHUB_TOKEN environment variable for best results.",
    inputSchema: {
        packageId: z
            .string()
        ...
        pattern: z
            .string()
            .describe("Search text to find in source files (GitHub Search syntax is supported)"),
    },
    ...
    try {
        const searchData = await fetchGitHubAPI(`/search/code?q=${encodeURIComponent(query)}`);
...
async function main() {
    ...
    console.error(`📦 Serving ${PACKAGES.length} packages from GitHub (${reposList})`);
    if (!GITHUB_TOKEN) {
        console.error("⚠️  GITHUB_TOKEN is not set. API rate limits will apply for 'search_source' and 'get_package_source' listing.");
    }
}
main().catch((error) => {
